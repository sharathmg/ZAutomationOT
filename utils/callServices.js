	...
	return {
			...
			if (bUpdate) {
				...
				oModelService.update(
					"/MAIN_TABLESet" + sParam, mParameter, {
						async: true,
						headers: {
							"if-match": "W/\"datetimeoffset'2017-05-05T09%3A15%3A05.6050000Z'\""
						},
						success: function(oResult, mHeader) {
							// if(mParameter.Projectkey === "WFLW" || mParameter.Projectkey === "WFLW" ||mParameter.Projectkey === "ENH")
							// {
		...
		callAttachmentService: function(oFileUploader, sField, readAttachPara, oViewModel, attachPropList, attachTableVisib) { // fnCallReadAttachments
			...
			var sFileUploadUrl =
				...
				"/sap/opu/odata/SAP/ZATTACHMENT_EXCEL_SRV/ZTATTACHMENTSET";
			...
			oFileUploader.attachUploadComplete(function(oEvent) {
				...
				oRef.hideBusyIndicator();
			});
			var sCsrfToken = "";
			//SOC Writwick 18 July 2018
			jQuery.ajax({
				url: "/sap/opu/odata/SAP/ZATTACHMENT_EXCEL_SRV", //EOC Writwick 18 july 2018
				method: "GET",
				headers: {
					"X-CSRF-Token": "fetch"
				},
		...
		readAttachmentList: function(mParameter, oViewModel, attachPropList, attachTableVisib) {
			...
			// sParam += "&$format=json";
			var sParam = "?$filter=FIELDNAME eq '" + mParameter.FIELDNAME +
				"' and PROCESSID eq '" + mParameter.PROCESSID +
				"' and PROJECTID eq '" + "TOYOTA" +
				"' and PROJECTKEY eq '" + mParameter.PROJECTKEY +
				"' and REPID eq '" + mParameter.REPID +
				"' and STEPNO eq '" + mParameter.STEPNO + "'&$format=json";
			//EOC Writwick 18 July 2018
			...
			var sServiceURL = "/sap/opu/odata/SAP/ZATTACHMENT_EXCEL_SRV/ZTATTACHMENTSET";
			//EOC Writwick 18 July 2018
			var oRes = jQuery.sap.sjax({
		...
		deleteAttachment: function(filePath) {
			...
			var sServUrl = "/sap/opu/odata/SAP/ZATTACHMENT_EXCEL_SRV/";
			//EOC Writwick 18 July 2018
